...
; new
; Select all from one tuple table
...
; new
; Select all from multiple duplicate table
(test (SELECT * FROM Favourite)
      '(("Name" "Things" "Age")
        ("Jack" "Swim" "18")
        ("Jack" "Food" "19")
        ("Tom" "Shot" "20")
...
(test (SELECT * FROM [Person "P"] [Teaching "T"] [Games "G"])
      '(("P.Name" "Age" "LikesChocolate" "T.Name" "Course" "G.Name" "Price")
        ("David" 20 #t "David" "CSC324" "League" "0")
        ("David" 20 #t "Paul" "CSC108" "League" "0")
        ("David" 20 #t "David" "CSC343" "League" "0")
        ("Jen" 30 #t "David" "CSC324" "League" "0")
        ("Jen" 30 #t "Paul" "CSC108" "League" "0")
        ("Jen" 30 #t "David" "CSC343" "League" "0")
        ("Paul" 100 #f "David" "CSC324" "League" "0")
        ("Paul" 100 #f "Paul" "CSC108" "League" "0")
        ("Paul" 100 #f "David" "CSC343" "League" "0")))

; new
; Select all from product of a multiple duplicate table
; and a one row table
(test (SELECT * FROM [Favourite "F"] [Games "G"])
      '(("F.Name" "Things" "Age" "G.Name" "Price")
        ("Jack" "Swim" "18" "League" "0")
        ("Jack" "Food" "19" "League" "0")
        ("Tom" "Shot" "20" "League" "0")
        ("Tom" "Swim" "20" "League" "0")
        ("Sam" "Food" "19" "League" "0")))
...
; new
; Select all from two product where one table is empty
...
; new
; Same case test on "P.Name" and "Name"
(test (SELECT '("Name") FROM [Person "P"] [Empty_table "E"])
      '(("Name")
        ("David")
        ("Jen")
        ("Paul")
...
(test (SELECT * FROM [Empty_table "E"] [Empty_table "E1"])
      '(()
        ()))

; new
; Take the product of an empty table with itself multiple times
(test (SELECT * FROM [Empty_table "E"] [Empty_table "E1"] [Empty_table "E2"])
      '(()
        ()))
...
; new
; Select some from the produc of a literal table with an Empey table
(test
 (SELECT '("Age")
         FROM ['(("Age" "A" "Name" "D")
                 (1 "Hi" 5 #t)
...
; new
; check same condition after join an empty table
(test (SELECT * 
              FROM [Person "P"] [Empty_table "E"]
              WHERE (< 50 "Age"))
      '(("Name" "Age" "LikesChocolate")
        ("Paul" 100 #f)))
...
; new
; Constant true condition for empty table
(test (SELECT *
              FROM Empty_table
              WHERE #t)
      Empty_table)
...
; new
; Constant false compound condition for empty table
(test (SELECT *
              FROM Person
              WHERE #f)
      '(("Name" "Age" "LikesChocolate")))
...
; new
; check same condition by join an empty
(test (SELECT '("C" "B")
              FROM ['(("A" "B" "C") 
                      (1 2 3)
                      (3 10 40)
...
; new
; Order on two tables, one is empty
(test (SELECT *
              FROM [Person "P"] [Empty_table "E"]
              ORDER BY "Age")
      '(("Name" "Age" "LikesChocolate")
...
; new
; Same case but combine with an empty table
(test
 (SELECT '("P.Name" "Course" "LikesChocolate")
...
; two tables without name replacement
(test (SELECT * FROM Person Count_num)
      '(("Name" "Age" "LikesChocolate" "Count")
        ("David" 20 #t "one")
        ("David" 20 #t "two")
        ("Jen" 30 #t "one")
        ("Jen" 30 #t "two")
...
; three tables without name replacement
(test (SELECT * FROM Person Count_num Fav)
      '(("Name" "Age" "LikesChocolate" "Count" "Things")
        ("David" 20 #t "one" "Swim")
        ("David" 20 #t "one" "Food")
        ("David" 20 #t "two" "Swim")
        ("David" 20 #t "two" "Food")
...
; two table join with NoTuple table
(test (SELECT * FROM Person NoTuple)
      '(("Name" "Age" "LikesChocolate" "Attr1" "Attr2")))

...
#| should not be valid
; Select some from two product where one table is empty
(test (SELECT '("P.Name") FROM [Person "P"] [Empty_table "E"])
      '(("P.Name")
        ("David")
        ("Jen")
        ("Paul")
...
; check same condition buy join an empty
(test (SELECT '("C" "B")
              FROM ['(("A" "B" "C") 
                      (1 2 3)
                      (3 10 40)
